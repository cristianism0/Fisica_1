...
def main():
    ...
    try:
        df = ut.utils_df_maker(name)
        df_columns = ut.utils_columns_extract(df)

    except Exception as e:
        raise e(f"Planilha não indentificada: {e}. Por favor, verifique a tabela!")
    ...
    while True:
        ...
        match option:
            ...
            case '1':
                ...
                print(result)
                ut.utils_csv_creater(result, path = DATA_PATH)
                    
            case '2':
                try:
                    print("Nome do arquivo com [mean] e [std]:", ut.utils_list_data_dir(DATA_PATH))
                    file_name = input()
                    result = mt.calc_exp_values(file_name)
            ...
            case '3':
                ...
                match choice:
                    ...
                    case '2':
                        try:
                            A, B, oA, oB, W = map(float, input("...").split(','))
                    ...
                    case '4':
                        try:
                            A, B, oA, oB, W, p= map(float, input("...").split(','))
